...
static STR isVblk(STR type, STR value) {
    if (type == "Vblk") {
        return "";
    }
    return value;
...
static void writeHpp(STR path, STR author, STR clsName, VSTR headers, VSTR value, VSTR type, STR refCntCls) {
    ...
    VSTR getArray, setArray, valueArray, WlStArray, DiStArray;
    ...
    for (auto i = 0; i < length; ++i) {
        ...
        WlStArray.emplace_back("\tmutable " + tp2BkTp(type[i]) + " _" + value[i] + "WlStBlk;\n");
        DiStArray.emplace_back("\tmutable " + tp2BkTp(type[i]) + " _" + value[i] + "DiStBlk;\n");
    }
...
static void writeCpp(STR path, STR author, STR clsName, VSTR headers, VSTR value, VSTR type, STR refCntCls) {
    ...
    for (auto i = 0; i < length; ++i) {
        ...
        setterArray.emplace_back("\tif (_" + value[i] + "WlStBlk) {\n");
        setterArray.emplace_back("\t\t_" + value[i] + "WlStBlk(" + isVblk(type[i], value[i]) + ");\n\t}\n\n");
        setterArray.emplace_back("\t_" + value[i] + " = " + value[i] + ";\n\n");
        setterArray.emplace_back("\tif (_" + value[i] + "DiStBlk) {\n");
        setterArray.emplace_back("\t\t_" + value[i] + "DiStBlk(" + isVblk(type[i],value[i]) + ");\n\t}\n}\n\n");
    }
    ...
    for (auto i = 0; i < length; ++i) {
        paraInitArray.emplace_back("_" + value[i] + "WlStBlk(NULL),\n");
    }
    for (auto i = 0; i < length; ++i) {
        paraInitArray.emplace_back("_" + value[i] + "DiStBlk(NULL),\n");
    }
    paraInitArray[paraInitArray.size() - 1][paraInitArray[paraInitArray.size() - 1].length() - 2] = ' ';
    paraInitArray.emplace_back("{\n\n}\n\n");
...
void WILLDID::make(STR path, STR author, STR clsName, VSTR headers, VSTR value, VSTR type, STR refCntCls) {
